...
export SAN=grpc-with-quarkus.app-cees-bcu-app.apps.freubel.sandbox2538.opentlc.com
export NS_SCMP=app-cees-bcu-smcp
...
oc delete secret igw-credential -n ${NS_SCMP}
oc create -n ${NS_SCMP} secret generic igw-credential --from-file=tls.key=server.key --from-file=tls.crt=server.crt --from-file=ca.crt=CA.crt
